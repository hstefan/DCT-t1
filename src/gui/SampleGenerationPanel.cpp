...
SampleGenerationPanel::SampleGenerationPanel(scv::Point pi, unsigned int width, CoefTable* table)
	...
	table_ptr(table), len_label(scv::Point(W_SPACING, H_SPACING), "Tamanho"),
	type_label(scv::Point(W_SPACING, H_SPACING*2 + len_label.getHeight()), "Tipo"),
	sample_box(scv::Point(100, H_SPACING + len_label.getHeight()), COMBO_WIDTH),
	len_box(scv::Point(100, len_label.getWidth()) + H_SPACING + len_label.getHeight()*2, COMBO_WIDTH),
	sample_button(this, len_label.getHeight()*3 + H_SPACING, getWidth() - 2*W_SPACING)
{
